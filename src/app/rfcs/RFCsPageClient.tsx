...
export default function RFCsPageClient({ session }: RFCsPageClientProps) {
    ...
    const [availableRepos, setAvailableRepos] = useState<RepoOption[] | null>(null)
    const [selectedRepo, setSelectedRepo] = useState<RepoOption | null>(null)
    const rfcsAbortControllerRef = useRef<AbortController | null>(null)
    const [selectedStatuses, setSelectedStatuses] = useState<Set<RFC["status"]>>(new Set(["open"]))
    ...
    function repoQueryParams(repo?: RepoOption | null): string {
        return repo ? `owner=${encodeURIComponent(repo.owner)}&repo=${encodeURIComponent(repo.name)}` : ""
    }
    ...
    async function loadRFCs(repo?: RepoOption | null) {
        ...
        try {
            ...
            const response = await fetch(`/api/rfcs${repoParams ? `?${repoParams}` : ""}`, {
            ...
            if (missingByRepo.size > 0) {
                ...
                const allCounts = Object.assign({}, ...countEntries) as Record<string, number>
                setRfcs(
                    (prev) =>
                        prev?.map((rfc) => {
    ...
    const repoSelectorElement = (
        <div className="flex items-center gap-2">
            {availableRepos && availableRepos.length > 0 ? (
                ...
                <div className="text-sm font-medium text-gray-50 flex items-center gap-2" aria-hidden>
                    All repositories
        ...
        <div className="mx-auto min-h-screen max-w-240 px-4 sm:px-8 py-6 sm:py-12">
            ...
            {missingScopes.length > 0 && (
                <div className="mb-6 border border-magenta/30 bg-magenta-light rounded-md px-4 py-3 text-sm text-foreground flex flex-wrap items-center gap-x-3 gap-y-1.5">
                    <span>
                        Your GitHub sign-in predates support for team-requested reviews.<br/>
                        <a href="/api/auth/signout" className="underline font-medium text-magenta hover:no-underline">
                            Log out
                        </a>{" "}
                        and sign back in to fix this - GitHub will ask you to grant the new required scope (
                        <code className="font-mono text-xs">{missingScopes.join(", ")}</code>).
                    </span>
                </div>
            )}
                        ...
                        >
                            {selectedAuthor ? (
                                <span className="flex items-center gap-1.5">
                                    <img
                                        src={authors.find((a) => a.login === selectedAuthor)?.avatar}
                                        alt={selectedAuthor}
                            ...
                            {authorDropdownOpen && (
                                ...
                                >
                                    ...
                                    <div className="max-h-64 overflow-y-auto">
                                        ...
                                        {authors
                                            .filter((a) =>
                                                a.login.toLowerCase().includes(authorSearchQuery.toLowerCase()),
                                            )
                                            .map((author) => (
                                                <button
                                                    key={author.login}
                                                    ...
                                                    className={`w-full text-left px-4 py-2.5 text-sm border-b border-gray-20 last:border-b-0 hover:bg-yellow-light transition-colors flex items-center gap-2 ${
                                                        selectedAuthor === author.login ? "bg-gray-5 font-medium" : ""
                                                    }`}
                                                >
                                                    <img
                                                        src={author.avatar}
