    ...
    const rfcPulls = await Promise.all(
        pulls.map(async (pr) => {
            ...
            const hasRFCMarkdown = files.some(
                (file) => file.filename.startsWith("requests-for-comments/") && file.filename.endsWith(".md")
            )
...
export async function getRFCDetail(accessToken: string, prNumber: number): Promise<RFCDetail> {
    const octokit = await getOctokit(accessToken)
    ...
    const uniqueReviewers = Array.from(new Map(reviewers.map((r) => [r.login, r])).values())
